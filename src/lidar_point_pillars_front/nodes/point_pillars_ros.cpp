...
void PointPillarsROS::createROSPubSub()
{
  ...
  //sub_points_ = nh_.subscribe<sensor_msgs::PointCloud2>("/original_view", 1, &PointPillarsROS::pointsCallback, this);
  pub_objects_ = nh_.advertise<autoware_msgs::DetectedObjectArray>("/detection/lidar_detector/objects_front", 1);
}
...
void PointPillarsROS::pubDetectedObject(const std::vector<float>& detections, const std_msgs::Header& in_header)
{
  ...
  ROS_INFO("publishing %d", num_objects);
